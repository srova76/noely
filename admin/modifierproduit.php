<?php
    ...
    $dn=new DadabeNoely();
    $id=$_GET["id"];
    $produit=$dn->select("*","cadeau","idCadeau='$id'");
    if(count($produit) !=0){
        $produit = $produit[0];
    }
?>
...
<head>
    ...
    <title>Admin - Modification Produit</title>
</head>
<body>
    <div class="navbar navbar-default navbar-fixed-top"role="navigation">
        ...
        ?>
    <div class='container'>
        <form class="form-signin" method="POST" action="modificationproduit.php">
            <legend>Formulaire modification Produit</legend>
            <div class="form-group">
                <input type="hidden" name="cadeau" value="<?php echo $produit["idCadeau"];?>">
                <label for="inputnom">Nom: </label>
                <input name="inputnom" id="inputnom" type="text" class="form-control" placeholder="Nom" value='<?php echo $produit["nomCadeau"];?>' required>
                <label for="inputpoint">Point: </label>
                <input name="inputpoint" id="inputpoint" type="number" min='0' class="form-control" placeholder="Point" value='<?php echo $produit["point"];?>' required>
